...
/*   Updated: 2025/01/03 18:36:47 by abdsalah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void fill_stack(t_item **stack, int *numbers, int size)
{
    ...
    while (i >= 0)
    {
        new_node = setup_node(numbers[i], 0, 0, false, false, NULL);
        if (!new_node)
        {
            free_stack(*stack);
            free(numbers);
...
t_item *setup_node(int val, int index, int push_cost, bool above_median, bool cheapest, t_item *target)
{
	t_item *node;
